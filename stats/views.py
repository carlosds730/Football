...
def season(request, season_num, order='elo'):
    ...
    try:
        active_players.sort(key=lambda x: getattr(x, order), reverse=reverse)
        inactive_players.sort(key=lambda x: getattr(x, order), reverse=reverse)
    ...
    return wrapper(request, 'season.html',
                   {'active_players': active_players, 'inactive_players': inactive_players, 'season_num': season_info,
                    'order': original_order, 'order_by': ORDER_BY})
